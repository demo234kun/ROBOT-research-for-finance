...
| Endpoint | `GET https://app-csctj2dvhmo1-api-ra5ErGpGM8wa-gateway.appmiaoda.com/traffic/v1/bound` |
| Auth | `platform_managed`（`X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}`） |
| Content-Type | `application/json`（GET 请求，参数通过 Query String 传递） |
| third_part_domain | `app-csctj2dvhmo1-api-ra5ErGpGM8wa-gateway.appmiaoda.com` |
...
async function queryBoundTraffic(
  ...
  const response = await fetch(
    `https://app-csctj2dvhmo1-api-ra5ErGpGM8wa-gateway.appmiaoda.com/traffic/v1/bound?${params.toString()}`,
    {
      method: "GET",
      headers: {
